 ...
 * @version 2018.04.17
 */
public class SistemaGestion implements Sujeto
{
    private Almacen almacen;
    private Comercial comercial;
    private Usuarios  gestionUsuarios;
    private Caja caja;
    private ArrayList<Observador> observadoresUsuarios;
    ...
    public SistemaGestion()
    {
        ...
        observadoresUsuarios = new ArrayList();
        cargarDatos();
    }    
    
    /**
     * Añade un observador a la lista de observadores de usuarios
     * @param obs observador que se debe añadir
     */
    public void addObserverUsuarios(Observador obs){        
        addObserver(obs,observadoresUsuarios);
    }
    
    /**
     * Elmina un observador de una lista de observadores de usuarios
     * @param obs observador que se debe quitar
     */
    public void removeObserverUsuarios(Observador obs){
        removeObserver(obs,observadoresUsuarios);
    }
    
    /**
     * Añade un observador a una lista
     * @param obs observador que se debe añadir
     * @param observadores lista a la que se debe añadir el observador
     */
    public void addObserver(Observador obs,List<Observador> observadores){        
        observadores.add(obs);
    }
    
    /**
     * Quitar un observador de una lista
     * @param obs observador que se debe quitar
     * @param observadores lista de la que se debe quitar el observador
     */
    public void removeObserver(Observador obs,List<Observador> observadores){
        observadores.remove(obs);
    }
    
    /**
     * Llama al método update de los observadores en la lista
     * @param observadores lista de observadores que se quiere actualizar
     */
    public void notifyObservers(List<Observador> observadores){
    ...
    {       
        boolean confirmado = gestionUsuarios.login(identificacion,clave);
        notifyObservers(observadoresUsuarios);
        return confirmado;
    }
    ...
    {
        if(gestionUsuarios.getEmpleadoActual()==null){
            return "desconocido";
        }else{
    ...
    {
        return almacen.añadirProducto(tipo,codigoDeProducto,marca,modelo,color,precio,cantidad);
    }    
    
    public void addCaracteristica(String caracteristica,String codigoDeProducto){
        almacen.getProductoPorCp(codigoDeProducto).addCaracteristica(caracteristica);
    }
    ...
    public boolean quitarProductoDeCompra(int numeroDeFicha, String idProducto,int cantidad)
    {
       ...
       if(caja.quitarProducto(numeroDeFicha,idProducto,cantidad,precio))
       {
           ...
           idProducto,gestionUsuarios.getEmpleadoActual().getIdentificacion());
           System.out.println("pase por aqui5");
           return true;
       }
       else
       {
    ...
    public void analizarFinanciacion(int numeroDeFicha,double nomina,int plazo)
    {
        ...
        String idUsuario = gestionUsuarios.getEmpleadoActual().getIdentificacion();
        int numeroFichaFin = financiacion.crearFicha(idCliente,idUsuario,numeroDeFicha);
                
         if(financiacion.analizarFinanciacion(numeroFichaFin, nomina, plazo,totalCompra))
        {
    ...
    {
        System.out.println("pasa1.0");
        int productosComprados = caja.getFichaPorNumero(numeroFichaDeCompra).getListaProductos().get(idProducto);
        System.out.println("pasa1.00");
        String idCliente = postVenta.getFicha(numeroFicha).getIdCliente();
        gestionUsuarios.getCliente(idCliente).setAccion("Ficha de devolucion:"+numeroFicha+" inicia devolucion de producto "+idProducto,gestionUsuarios.getEmpleadoActual().getIdentificacion());
        System.out.println("pasa1.000");
        if(postVenta.añadirProducto(numeroFicha,idProducto,cantidad,productosComprados,numeroFichaDeCompra))
        {
            System.out.println("pasa1.1");
            almacen.setRecepcionDePedidos(idProducto,cantidad);
            return true;
        }
        else
        {
            System.out.println("pasa1.2");
            return false;
        }
    }
    ...
    {
        postVenta.completarDevolucion(numeroFicha);
        gestionUsuarios.getCliente(postVenta.getFicha(numeroFicha).getIdCliente()).setAccion("Ficha de devolucion:"+numeroFicha+
